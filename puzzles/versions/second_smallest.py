s1="TestPackage-1.2.18"
s2="TestPackage-1.2.215"
s3="TestPackage-1.2.210"

#-1 first larger, 0 equal, 1 second greater
def compare_versions(s1,s2):
#  print "   comparing", s1, " and", s2
  version1=s1.split("-")[1]
  v_parts1=version1.split(".")

  version2=s2.split("-")[1]
  v_parts2=version2.split(".")

  for i in range(len(v_parts1)):
    if int(v_parts1[i])>int(v_parts2[i]):
        return -1
    elif int(v_parts1[i])<int(v_parts2[i]):
        return 1

  return 0

s1="TestPackage-1.2.18"
s2="TestPackage-1.2.215"
s3="TestPackage-1.5.15"
s4="TestPackage-2.5.412"
s5="TestPackage-3.2.4"

sl=[s1, s2, s3,  s4, s5] 

smallest=None 
second=None

while sl:
  current=sl.pop()
  if not smallest:
    smallest=current
    #print "no smallest, assigning", smallest
  elif compare_versions(current,smallest)>0: 
    #print "new smallest: ",current,"new second: ",smallest,"third: ",second

    second=smallest
    smallest=current
  elif not second or compare_versions(current,second)>0:
      print "replacing second",second, "with " , current
      second=current
    
print "second",second

